...
public class iBeaconClass {
	...
	public static final double [][] nodeCoordinate= {
		{0.0, 0.0}, {3.3, 0.3}, {3.3, 10.0}, {1.5, 7.5}, {0.0, 0.0}, {3.3, 0.3}, {3.3, 10.0}, {3.3, 6.2}, {3.3, 6.2}, {3.3, 6.2}
	};
    ...
    public static int ledId = 0x00000000;
    public static int advYaw, advPitch, advRoll;
    ...
    public static iBeacon fromScanData(BluetoothDevice device, int rssi,byte[] scanData) {
		...
		advYaw = (scanData[startByte+12] & 0x000000ff) << 24 | (scanData[startByte+11] & 0x000000ff) << 16 |
				(scanData[startByte+10] & 0x000000ff) << 8 | (scanData[startByte+9] & 0x000000ff);
		
		advPitch = (scanData[startByte+16] & 0x000000ff) << 24 | (scanData[startByte+15] & 0x000000ff) << 16 |
				(scanData[startByte+14] & 0x000000ff) << 8 | (scanData[startByte+13] & 0x000000ff);
		
		advRoll = (scanData[startByte+20] & 0x000000ff) << 24 | (scanData[startByte+19] & 0x000000ff) << 16 |
				(scanData[startByte+18] & 0x000000ff) << 8 | (scanData[startByte+17] & 0x000000ff);
		
		
		Log.i("euler","advYaw = " + Float.intBitsToFloat(advYaw) + "  advPitch = " + Float.intBitsToFloat(advPitch)
				+ "  advRoll = " + Float.intBitsToFloat(advRoll));
		
		sportsUnitCollectData.jump = (int)scanData[startByte+21];
		Log.e("jump", "jump  =  " + sportsUnitCollectData.jump);
...
//		iBeacon.major = (scanData[startByte+25] & 0xff) * 0x100 + (scanData[startByte+26] & 0xff);
//		iBeacon.minor = (scanData[startByte+27] & 0xff) * 0x100 + (scanData[startByte+28] & 0xff);
//		if(iBeacon.major == 8)
//		{
//			
//			Log.i("dev_id", "major" + iBeacon.major + "minor" + iBeacon.minor);
//		}
//		
//		//Log.i("dev_id", "major" + iBeacon.major + "minor" + iBeacon.minor);
//		
//		iBeacon.txPower = (int)scanData[startByte+29]; // this one is signed
...
//		iBeacon.rssi = (int) data.rssi;
		...
		byte[] proximityUuidBytes = new byte[4];
		System.arraycopy(scanData, startByte+9, proximityUuidBytes, 0, 4); 
		String hexString = bytesToHexString(proximityUuidBytes);
		StringBuilder sb = new StringBuilder();
		sb.append(hexString.substring(0,8));
		iBeacon.proximityUuid = sb.toString();
		
//		Log.i("LedId", "led_id : " + iBeacon.proximityUuid + "  advRadius = " + Float.intBitsToFloat(advRadius) + "  advTheta = " + Float.intBitsToFloat(advTheta)
//				+ "  advAzimuth = " + Float.intBitsToFloat(advAzimuth));
