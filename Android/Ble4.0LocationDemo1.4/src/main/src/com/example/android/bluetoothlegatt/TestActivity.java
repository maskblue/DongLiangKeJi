...
//						}
						...
						tv.setText(tv.getText() + "\n jump: " + (iBeaconClass.sportsUnitCollectData.jump) );
						
						tv.setText(tv.getText() + "\n dash: " + (iBeaconClass.sportsUnitCollectData.dash) );
						
						tv.setText(tv.getText() + "\n veer: " + (iBeaconClass.sportsUnitCollectData.veer) );
						
						if(Float.intBitsToFloat(iBeaconClass.advYaw) < 1.0)
						{
							tv.setText(tv.getText() + "\n yaw: 0" + df.format(Float.intBitsToFloat(iBeaconClass.advYaw)) );
						}else{
							tv.setText(tv.getText() + "\n yaw: " + df.format(Float.intBitsToFloat(iBeaconClass.advYaw)) );
						}
						
						if(Float.intBitsToFloat(iBeaconClass.advPitch) < 1.0 && Float.intBitsToFloat(iBeaconClass.advPitch) > 0.0)
						{
							tv.setText(tv.getText() + "\n pitch: 0" + df.format(Float.intBitsToFloat(iBeaconClass.advPitch)) );
						}
						else if(Float.intBitsToFloat(iBeaconClass.advPitch) < 0.0 && Float.intBitsToFloat(iBeaconClass.advPitch) > -1.0)
						{
							tv.setText(tv.getText() + "\n pitch: -0" + df.format(Math.abs(Float.intBitsToFloat(iBeaconClass.advPitch))) );
						}else {
							tv.setText(tv.getText() + "\n pitch: " + df.format(Float.intBitsToFloat(iBeaconClass.advPitch)) );
						}
						
						if(Float.intBitsToFloat(iBeaconClass.advRoll) < 1.0 && Float.intBitsToFloat(iBeaconClass.advRoll) > 0.0)
						{
							tv.setText(tv.getText() + "\n roll: 0" + df.format(Float.intBitsToFloat(iBeaconClass.advRoll)) );
						}
						else if(Float.intBitsToFloat(iBeaconClass.advRoll) < 0.0 && Float.intBitsToFloat(iBeaconClass.advRoll) > -1.0)
						{
							tv.setText(tv.getText() + "\n roll: -0" + df.format(Math.abs(Float.intBitsToFloat(iBeaconClass.advRoll))) );
						}else {
							tv.setText(tv.getText() + "\n roll: " + df.format(Float.intBitsToFloat(iBeaconClass.advRoll)) );
						}
						
						//pointView.setRotation(Orientation.get_pose()[0] + 90);
						//pointView1.setRotation(Orientation.get_pose()[0] + 90);
						
						pointView.setRotation(Float.intBitsToFloat(iBeaconClass.advYaw));
						pointView1.setRotation(Float.intBitsToFloat(iBeaconClass.advYaw));
					}
				});
			}
		}, 0, 100);
    ...
    private void setBeaconPos(View iv, float x, float y) {
    	...
    	iv.setY((float) (y * App.H_PX_P_M - 10));
    }
    ...
    private void adjustLineViewPosition() {
    	...
    	//lineView.setX((float) (1.5 * App.W_PX_P_M));
    	//lineView.setY((float) (1.5 * App.H_PX_P_M));
	...
	public void onFinish() {
		runOnUiThread(new Runnable() {
            ...
            public void run() {
            	...
            	{	
            		x = iBeaconClass.nodeCoordinate[iBeaconClass.nodeFlags][0] + radius * Math.cos(iBeaconClass.ToR(theta - 5));
            		y = iBeaconClass.nodeCoordinate[iBeaconClass.nodeFlags][1] + radius * Math.sin(iBeaconClass.ToR(theta - 5));
            	}
            	...
            	y = ConvertUtils.geo2Screen(y, ConvertUtils.TYPE_HEIGHT);
...
//            		pointView.startAnimation(animation);
            		...
            		xAnimator.setFloatValues(pointView.getX(), (float)x);
            		yAnimator.setFloatValues(pointView.getY(), (float)y);
...
//                	pointView.setY((float) y);
            	...
            	//mGLSurfaceView.onMpu6050Sensor(yaw, pitch, roll);
            }
        });
	}
